...
export function DonutChartRenderer({ properties }: DonutChartRendererProps) {
  const chartData = properties?.donutChartData || [
  ...
  const variant = properties?.donutChartVariant || 'default';
  const title = properties?.donutChartTitle || 'Title goes here';
  const showTitle = properties?.showTitle !== false;
  const showLegend = properties?.showDonutLegend !== false;
  const showLabels = properties?.showDonutLabels !== false;
  const showPercentages = properties?.showDonutPercentages !== false;
  const buttons = properties?.donutChartButtons || [];
  const kpis = properties?.donutChartKpis || [];
  const innerRadius = properties?.donutInnerRadius || 40;
  const outerRadius = properties?.donutOuterRadius || 80;

  const defaultColors = ['#4F46E5', '#7C3AED', '#06B6D4', '#8B5CF6', '#EC4899'];
  const donutColors = properties?.donutColors || defaultColors;
