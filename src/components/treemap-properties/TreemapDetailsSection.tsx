...
interface TreemapDetailsSectionProps {
  element: Element;
  updateElementProperties: (id: string, properties: Partial<Element['properties']>) => void;
  onOpenStyleDialog?: () => void;
}

export function TreemapDetailsSection({ element, updateElementProperties, onOpenStyleDialog }: TreemapDetailsSectionProps) {
        ...
        {showTitle && (
          <div>
            ...
            <Input
              value={treemapTitle}
              onChange={(e) => 
                updateElementProperties(element.id, { treemapTitle: e.target.value })
              }
              placeholder="Enter treemap title"
              className="mt-1"
            />
          </div>
        )}
